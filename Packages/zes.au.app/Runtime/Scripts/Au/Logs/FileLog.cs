...
namespace Au.Logs
{
    internal class FileLog : ILogHandler
    {
        public FileLog(string name, int maxSize, int maxCount)
        {
            this.name = name;
        ...
        {
            writer.WriteLine($"[{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")}] [{LogTypeNames.GetName(type)}] {message}");
